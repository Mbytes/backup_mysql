...
#Extraemos Usuarios defecto Debian
...
cat "${TEMPTABLES}" | awk -v DUMP=${DUMP} -v USER=${USER} -v PWD=${PWD} -v FECHA=${FECHA} -v PREFIX=${HOST} '{print "mysqldump --compact --insert-ignore --single-transaction -hlocalhost -u" USER " -p" PWD " --quote-names " $1 " -r " DUMP "/" PREFIX "_" $1 "_" FECHA ".sql" }' > ${TMPDUMP}
sh "${TMPDUMP}"

#Realizamos volcados structuras
cat "${TEMPTABLES}" | awk -v DUMP=${DUMP} -v USER=${USER} -v PWD=${PWD} -v FECHA=${FECHA} -v PREFIX=${HOST} '{print "mysqldump -d -hlocalhost -u" USER " -p" PWD " --quote-names " $1 " -r " DUMP "/" PREFIX "_STRUCT_" $1 "_" FECHA ".sql" }' > ${TMPDUMP}
sh "${TMPDUMP}"
